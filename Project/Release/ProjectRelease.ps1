...
function New-InitialBuildInfoJson
{
    ...
    return [ordered]@{
        ...
        commit   = ""
        builtUtc = ""
        notes    = "Build metadata will be completed by ProjectRelease.ps1."
    }
}
...
function Get-UpdatedReadmeContent
{
    ...
    if ($HasMarkers)
    {
        $ExistingBlock = [regex]::Match(
            $Readme,
            "(?s)<!-- PROJECTRELEASE:BEGIN -->.*?<!-- PROJECTRELEASE:END -->"
        ).Value
...
function Set-FinalBuildInfo
{
    ...
    Set-ObjectProperty `
        -InputObject $BuildInfo `
        -Name "notes" `
        -Value "Build metadata recorded by ProjectRelease.ps1."
